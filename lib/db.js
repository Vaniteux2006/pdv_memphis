...
async function createUser({ email, name, password, role, mustChangePassword, grupo, regiao, telefone }) {
  ...
  const user = {
    ...
    grupo: String(grupo || '').trim(), regiao: String(regiao || '').trim(), telefone: String(telefone || '').trim(),
  };
...
async function updateUser(id, fields) {
  ...
  const set = {};
  for (const k of ['name', 'grupo', 'regiao', 'telefone']) {
...
async function getConfig() {
  ...
  cfgEmVoo = (async () => {
    const c = await cols();
    cfgCache = (await c.config.findOne({ _id: 'singleton' })) || { senhaMensal: '', senhaSemanal: '' };
...
module.exports = {
  init,
  REGIOES, PONTOS_EXTRA, PRE_AVALIACOES,
  LIMITE_SEMANAL, LIMITE_MENSAL, IMAGENS_POR_FOTO, semanaISO, mesDe,
  findUserByEmail, findUserById, checkPassword, listUsers, createUser, updateUser, changeOwnPassword, setProvisionalPassword, setPassword, setUserActive, deleteUser,
