...
// que não dava pra somar). "Outros" é o escape — quem escolhe explica no campo de observação.
const MOTIVOS_RECUSA = ['Foto fora de foco', 'Exposição não é da marca', 'Falta etiqueta de preço',
  'Foto repetida', 'Produto errado', 'Exposição incompleta', 'Outros'];
...
function validaSenha(pw) {
  pw = String(pw || '');
  if (pw.length < 6) throw new Error('A senha precisa de ao menos 6 caracteres');
  const n = pw.toLowerCase();
  if (SENHAS_OBVIAS.has(n)) throw new Error('Essa senha é fácil demais de adivinhar — escolha outra');
  if (/^(.)\1+$/.test(pw)) throw new Error('Senha com um caractere só repetido não vale — escolha outra');
  // sequência corrida de dígitos, pra cima ou pra baixo ("456789", "87654321")
  if (/^\d+$/.test(pw) && ('01234567890'.includes(pw) || '09876543210'.includes(pw)))
...
function bucketsEntre(de, ate, por) {
  ...
  if (isNaN(d) || isNaN(fim) || d > fim) return chaves;
  const visto = new Set();
  while (d <= fim) {
...
async function aderencia({ de, ate } = {}) {
  ...
  const comGrupoDoBanco = [
    ...
    { $set: { grupoBanco: { $ifNull: [{ $arrayElemAt: ['$_banco.grupo', 0] }, ''] } } },
  ];
