...
// Foto de perfil: só o publicId do Cloudinary. Devolve a anterior pro caller apagar o
// arquivo — trocar de foto não pode deixar a antiga acumulando no storage.
async function setAvatar(id, avatar) {
  const c = await cols();
  const u = await c.users.findOne({ id }, { projection: { avatar: 1 } });
  if (!u) throw new Error('Usuário não encontrado');
  await c.users.updateOne({ id }, avatar ? { $set: { avatar } } : { $unset: { avatar: '' } });
  limpaCacheUsers();
...
async function deleteUser(id) {
  ...
  limpaCacheUsers();
  return { avatar: (u && u.avatar) || null }; // pro caller apagar a foto de perfil no storage
}
...
const ACOES = {
  ...
  EXCLUIU_TODAS_FOTOS: 'excluiu_todas_fotos',
...
// Apaga TODAS as submissões — fim de campanha ou limpeza depois de um teste. Leva o quadro
// de honra junto: as medalhas apontam pra fotos que deixam de existir. O `retorno` (histórico
// de recusas do promotor) fica, pela mesma razão que sobrevive à anonimização.
// Devolve as submissões pro caller apagar as imagens no storage.
async function excluirTodasSubmissions() {
  const c = await cols();
  const subs = await c.submissions.find({}, { projection: { _id: 0 } }).toArray();
  await c.submissions.deleteMany({});
  const r = await c.ranking.deleteMany({});
  return { subs, ranking: r.deletedCount || 0 };
}
...
async function resetarCadastros(executorId) {
  const c = await cols();
  // fotos de perfil das contas que vão sumir — colhidas ANTES, pro caller apagar no storage
  const avatars = (await c.users.find({ role: { $ne: 'admin' }, avatar: { $exists: true } }, { projection: { avatar: 1 } }).toArray())
    .map((u) => u.avatar);
...
module.exports = {
  ...
  findUserByEmail, findUserById, checkPassword, listUsers, countPendingSignups, createUser, updateUser, changeOwnPassword, setAvatar, setProvisionalPassword, setPassword, setUserActive, deleteUser,
  ...
  montarBackup, registrarBackup, listBackups, backupsVencidos, removerBackupsDoRegistro, resetarCadastros, excluirTodasSubmissions,
